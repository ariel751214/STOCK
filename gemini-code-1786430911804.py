...
st.caption("專為投資新手設計：自動分析『高成長飆股突破』與『穩健護城河領息』雙軌策略，內建嚴謹 1% 風控與停損依據教學")

# 側邊欄設定
st.sidebar.header("⚙️ 1. 股票與資金設定")
raw_ticker = st.sidebar.text_input("股票 / ETF 代號 (如 2330, 2412, NVDA, 0056)", value="2330").strip().upper()
        ...
        try:
            ...
            if df is None or df.empty or len(df) < 5:
                ...
                with tab_growth:
                    ...
                    # 知識庫：停損與風控依據說明
                    with st.expander("💡 為什麼這樣設定停損點與買入數量？（原理與依據）", expanded=False):
                        st.markdown("""
                        **一、 為什麼以「近 10 日最低價」為停損點？**
                        * **結構失效底線**：在專業動能交易體系中，突破前 10 天的低點是主力機構完成最後洗盤的防守防線。
                        * **真假突破驗證**：若股價跌破近 10 日低點，代表本次突破為「假突破」，機構買盤已撤守，必須第一時間果斷離場以保全本金。

                        **二、 為什麼設定「8% 為絕對硬停損上限」？**
                        * **虧損數學不對稱性**：賠 8% 只要漲 8.7% 就能回本；但若賠 20% 需要漲 25%，賠 50% 則需要翻倍（漲 100%）才能回本。將停損嚴格鎖在 8% 以內，才能確保永遠不傷及元氣。

                        **三、 為什麼嚴格遵守「1% 帳戶風險法則」？**
                        * **部位大小公式**：`建議買入股數 = (投資閒錢 × 1%) ÷ 每股承擔風險`。
                        * **安全保證**：哪怕這筆交易完全失敗並觸發停損，你的整體帳戶也只會損失剛好 1% 閒錢，連續做錯 10 次也不會危及資產！
                        """)
                ...
                with tab_dividend:
                    ...
                    if is_etf or (moat_pass and vnet_pass and (gross_margin >= 20.0 or is_large_cap)):
                        ...
                        if is_etf:
                            ...
                            * **存股買進建議**：不需理會短期的技術線型震盪，建議將資金拆分為 **12 ~ 24 個月**，採取**『定期定額分批買進、股息再投入』**策略。
                            """)
                        else:
                            st.markdown(f"""
                            * **護城河極深**：毛利率達 **{gross_margin:.1f}%** 且具備產業特許/龍頭地位，產品難以被對手取代。
                            * **營運穩健抗跌**：即便獲利沒有爆發性成長，但現金流與營收穩定，倒閉或大幅虧損風險極低。
                            * **存股買進建議**：適合採取**定期定額分批買進、長期領取股息**；股價因大盤回檔下跌時不停損，反而可適度逢低加碼。
                            """)
                    else:
                        st.warning("⚠️ **【領息需謹慎】** 該公司毛利較低或營運波動較大，長期存股需留意配息穩定度。")

                    # 知識庫：存股操作原則
                    with st.expander("💡 穩健存股的停損與買進原則是什麼？", expanded=False):
                        st.markdown("""
                        * **價格下跌不停損**：存股策略著重在「長期持有與領息」，股價因市場大盤回檔而下跌時不設技術停損，反而應視為降低持股成本的良機。
                        * **基本面質變才賣出**：僅在以下狀況考慮換股或出場：
                          1. 該公司**失去特許地位或護城河瓦解**（如毛利率連續數季嚴重崩跌）。
                          2. 公司**連續減少或停止發放股息**。
                          3. 找到殖利率更高且體質更健全的替代標的。
                        """)
